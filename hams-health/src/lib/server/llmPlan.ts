...
- ✅ steps에 title/imgSrc 같은 필드는 절대 포함하지 마라 (금지)
...
export async function llmMakePlan(input: RecommendInput): Promise<RecoPlanResolved> {
  ...
  try {
    ...
    // ✅ steps whitelist 추가
    const v1 = validatePlanSteps(raw, input.constraints?.time_min ?? 15);
    if (!v1.ok) {
      // 1회만 수정 재요청
      const fixedText = await llmFixPlan(text, v1.reason);
      const fixedPlan = JSON.parse(stripJsonCodeBlock(fixedText));

      const v2 = validatePlanSteps(fixedPlan, input.constraints?.time_min ?? 15);
      if (!v2.ok) {
        // ✅ 여기서 throw 하든지, 폴백하든지 선택
...
console.log("allowedStepIds============> ", allowedStepIds);
  const fixPrompt = `
너는 JSON 수리기다.
